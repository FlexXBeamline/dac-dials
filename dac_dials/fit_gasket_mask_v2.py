...
def run(args=None):
    usage = "dac.fit_gasket_mask integrated.expt integrated.refl [options]"
    ...
    refl = refl[
        0
    ]  # why is this a list? should probably raise an error if len(refl) ~= 1
    expt = expt[0]
    ...
    x_strong, y_stong = to_aperture_plane(
        *to_goniometer_frame(*calc_diffraction_vectors(expt, strong), phi0=params.phi0)
    )
    x_pred, y_pred = to_aperture_plane(
        *to_goniometer_frame(
    ...
    x, y = to_aperture_plane(
        *to_goniometer_frame(*calc_diffraction_vectors(expt, refl), phi0=model["phi0"])
    )
